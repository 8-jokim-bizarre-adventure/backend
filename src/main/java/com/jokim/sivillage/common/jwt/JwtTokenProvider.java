...
public class JwtTokenProvider {
     ...
     *
     * @param token
     * @return jwt토큰에서 추출한 사용자 UUID 반환 토큰에서 추출한 클레임에서 사용자 UUID를 추출하여 반환합니다.
     */
     ...
     * 3. 토큰에서 모든 claims 추출
     *
     * @param token 주어진 JWT 토큰에서 모든 클레임을 추출하여 반환합니다. 토큰의 서명을 확인하기 위해 사용할 서명 키(getSigningKey())를 설정하고
     *              토큰을 파싱하여 클레임들을 추출합니다.
     */
     ...
     * @return 클레임 정보와 사용자 정보를 기반으로 jwt 토큰 생성 클레임 정보, 사용자 ID, 생성 시간, 만료 시간 등을 설정하고, 서명 알고리즘과 서명 키를
     * 사용하여 토큰을 생성합니다. Access Token 역활
     */
    ...
    public String generateAccessToken(Authentication authentication) {
        ...
        Date expiration = new Date(
            now.getTime() + env.getProperty("jwt.token.access-expire-time", Long.class)
                .longValue());
    ...
    public String generateRefreshToken(Authentication authentication) {
        ...
        Date now = new Date();
        Date expiration = new Date(
            now.getTime() + env.getProperty("jwt.token.refresh-expire-time", Long.class)
                .longValue());
